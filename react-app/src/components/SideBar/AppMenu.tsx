...
const AppSubmenu = (props: PropsAppSubmenu) => {
    ...
    const renderLink = (item: any, i: any) => {
        ...
        const tabIndex: number | undefined = 0;
        if (item.to) {
            return (
            ...
            return (
                <a tabIndex={tabIndex} aria-label={item.label} onKeyDown={onKeyDown} role="menuitem" href={item.url} className="p-ripple" onClick={(e) => onMenuItemClick(e, item, i)} target={item.target}>
                    {content}
                </a>
            );
        }
...
export const AppMenu = (props: PropsAppMenu) => {
